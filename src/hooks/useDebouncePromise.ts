import DebouncePromise from "./usePromiseDebounce"
import { onlyResolvesLast } from "./useResolveLastPromise"
...
class DebounceCache<Fun extends (...args: any[]) => any> {
  ...
  _createDebouncedFunction(): CacheFunction<Fun> {
    let debouncedFunc: Fun = DebouncePromise(
      this.config.func,
      this.config.wait,
      this.config.options
    ) as any // TODO TS
  ...
  getDebouncedFunction(args: ArgumentsType<Fun>): CacheFunction<Fun> {
